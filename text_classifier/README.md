...
┌─────────────────────────────────────────┐
│  dataset.py                             │
│  load_csv() → train / val split         │
│  Vocabulary  (word→idx, label→idx)      │
│  make_dataloader() → padded batches     │
└────────────────────┬────────────────────┘
                     │  (text tensor, label tensor)
                     ▼
┌─────────────────────────────────────────┐
│  model.py  — TextClassifier             │
│  Embedding(vocab_size, embed_dim)       │
│       ↓                                 │
│  Bi-LSTM(hidden_dim, num_layers)        │
│       ↓  concat fwd + bwd hidden state  │
...
│  Adam optimizer  │  │  load_model_and_vocab()     │
│  CrossEntropy    │  │  predict() → label + probs  │
│  _run_epoch()    │  └────────────────────────────┘
└────────┬─────────┘            ▲
         │ saves                │ loads
         ▼                      │
